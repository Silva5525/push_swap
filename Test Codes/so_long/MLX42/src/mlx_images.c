...
/*   Updated: 2024/02/01 16:14:29 by wdegraf          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//= Private =//
// Diese Funktion ist ein typischer Teil eines effizienten Rendering-Prozesses, 
// bei dem viele geometrische Formen (in diesem Fall Dreiecke) 
// gesammelt und dann in einem Durchgang gezeichnet werden, um die Leistung zu verbessern.
...
// Diese Funktion ist wichtig für die Verwaltung von Texturen in einem Rendering-System,
// da sie sicherstellt, dass Texturen effizient und nur einmal gebunden werden. Sie ermöglicht 
// es auch, die Texturen an verschiedene 
// Textur-Slots zu binden, was für fortgeschrittene Rendering-Techniken erforderlich sein kann.
static int8_t mlx_bind_texture(mlx_ctx_t* mlx, mlx_image_t* img)
{
	...
	// If no free slot was found, flush the batch and assign the texture to the first 
	// available slot
	mlx_flush_batch(mlx);
 ...
 * to the screen.
 */
// Zusammengefasst, mlx_draw_instance ermöglicht es, ein Bild an einer bestimmten
// Position und in einer bestimmten Größe zu zeichnen, indem es die Textur des 
// Bildes bindet,
// die notwendigen Vertices vorbereitet und diese zum Rendering-Batch hinzufügt.
// Dieser Ansatz ist typisch für moderne Grafikprogrammierung, bei der 
// Batching-Techniken verwendet werden, 
// um die Anzahl der Rendering-Aufrufe zu minimieren und die Leistung zu optimieren.
...
// Zusammenfassend sorgt mlx_grow_instances für genügend Speicherplatz 
// für die Speicherung von Instanzen, definiert aber keine spezifischen 
// Eigenschaften von Instanzen wie Position, Größe oder Textur. Die 
// Definition einer mlx_instance_t Instanz im Sinne der Festlegung ihrer 
// Eigenschaften würde separat erfolgen, typischerweise bevor oder nachdem 
// dieser Speicherbereich 
// sichergestellt wurde, abhängig von der spezifischen Logik des Programms.
...
//= Public =//
// // 
// Die Funktion mlx_set_instance_depth aktualisiert die Tiefenposition 
// (Z-Koordinate) einer Instanz. Sie prüft zuerst, ob der neue Tiefenwert 
// sich vom aktuellen unterscheidet. Ist dies der Fall, wird der Wert aktualisiert.
// Die Funktion plant auch eine spätere Sortierung der Instanzen
...
// Zusammengefasst ermöglicht mlx_image_to_window das Zeichnen eines 
// Bildes an einer spezifischen Position in einem Fenster, wobei automatisch
// eine Tiefensortierung zur Vermeidung von Überlappungen berücksichtigt
// wird. Die Funktion behandelt auch Speicherreservierung und 
// Fehlermeldungen sorgfältig, um robustes Verhalten zu gewährleisten.
...
// Zusammenfassend erstellt mlx_new_image ein neues Bild mit den 
// spezifizierten Dimensionen, reserviert Speicher für dessen Pixel, erstellt 
// eine zugehörige OpenGL-Textur, konfiguriert diese Textur und registriert 
// das Bild im Grafikkontext. Dieser Vorgang ist zentral für Grafik- und 
// Spiele-Engines, die dynamisch Bilder laden, erstellen und darstellen müssen.
...
// Durch diese Schritte stellt mlx_delete_image sicher, dass ein Bildobjekt
//  und alle damit verbundenen Ressourcen korrekt und vollständig aus dem 
//  System entfernt werden, ohne Speicherlecks oder ungültige Referenzen zu 
//  hinterlassen. Dies ist ein wichtiger Prozess in Grafik- und Spiele-Engines,
//   um die Integrität des Systems während der 
// Laufzeit zu gewährleisten und Ressourcen effizient zu verwalten.
...
// Durch diese Schritte ändert mlx_resize_image die Größe des Bildes effektiv 
// und passt die entsprechende OpenGL-Textur an, um die neuen Dimensionen zu 
// reflektieren. Diese Funktion ist wichtig für Anwendungen, die dynamisch 
// die Größe von Bildern ändern müssen, sei es für UI-Elemente, Texturen in 
// Spielen oder jede andere 
// grafische Darstellung, die anpassbare Dimensionen erfordert.
