...
t_color_game init_game() {
  ...
  img4->instances[0].enabled = false; // disable score background for now
  {
    ...
    for (int i = 0; (const uint32_t)i < img4->width; i++) {
      for (int j = 0; (const uint32_t)j < img4->height; j++) {
...
void init_dinos(t_color_game * cg) {
  ft_lstadd_back(&cg->random_dinos, ft_lstnew(create_dino("./images/hero.png", cg->mlx)));
  ft_lstadd_back(&cg->random_dinos, ft_lstnew(create_dino("./images/hero.png", cg->mlx)));
  ft_lstadd_back(&cg->random_dinos, ft_lstnew(create_dino("./images/hero.png", cg->mlx)));
  ft_lstadd_back(&cg->random_dinos, ft_lstnew(create_dino("./images/hero.png", cg->mlx)));
}
